...
const Product = ({data}: Props) => {
    return <div className="w-full max-w-sm bg-white mb-35 max-h-500 ">
        <a href="detail/1">
            <img className="p-1 rounded-t-lg ml-5" src={data?.images[0].base_url} alt="product image" />
        </a>
        <div className="px-5 pb-5 ">
            <a href="">
                <h5 className="text-base font-roboto tracking-tight text-gray-900 dark:text-white mb-5"> {data.name}</h5>
            </a>
            <div className="flex items-center justify-between">
                <span className="text-base font-bold text-red-500 dark:text-white ">{data.price} đ</span>
                <span className="text-sm font-bold text-gray-400 dark:text-white ">{data.original_price} đ</span>
            </div>
